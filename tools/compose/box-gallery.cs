...
// build-cache bust: box-model gallery (G79 donut + G62 slot mirror) round 4 — wool arm via plan-cell BoxFiller, mouth-row seated (G63-C 1b)
...
{
    var id = $"p{c.P}·t{c.T}·{c.S}·s{c.Seed}";
    try
    {
...
static string FamTag(ShapeFamily f) => f.ToString().ToLowerInvariant();
    ...
    sectionsHtml.Append($"""
        <section class="fam">
          <div class="fam-head">
            ...
            <span class="fam-count">{list.Count} plan{(list.Count == 1 ? "" : "s")}</span>
          </div>
          <div class="grid">
    {cards}      </div>
        </section>
...
// ── the emitted approach-family catalog — one wool box filled with each family (single unit, no fan). This
// is what the fill menu can produce; the donut is the newly-admitted one (G79). ──
void Catalog(string header, string sub, IEnumerable<(string Name, ShapeFamily Fam, Func<EmittedApproach> Emit)> entries)
{
    var cards = new StringBuilder();
    int n = 0;
    foreach (var (name, fam, emit) in entries)
    {
        try
        {
            var svg = BuildSvg(WoolBoxEmitter.AsPlan(emit()));
            cards.Append($"""
                    <article class="card card--cat">
                      <div class="card-head"><span class="card-id">{Esc(name)}</span><span class="fam-badges"><span class="fam-badge fam-badge--{FamTag(fam)}">{Esc(fam.ToString())}</span></span></div>
    ...
    sectionsHtml.Append($"""
        <section class="fam">
          <div class="fam-head">
            <h2 class="fam-title">{Esc(header)}</h2>
            <span class="fam-sub">{Esc(sub)}</span>
            <span class="fam-count">{n} shape{(n == 1 ? "" : "s")}</span>
          </div>
          <div class="grid grid--cat">
    {cards}      </div>
        </section>
...
const int Cw = 2;
WoolBox Bx(int w, int h) => new(0, 0, w, h);
Catalog("The approach families the box emitter fills",
    "each is one wool box (green = wool room) filled with a base family — the escalation ladder the fill menu offers; the donut is the one G79 newly admits to production",
    new (string, ShapeFamily, Func<EmittedApproach>)[]
    {
        ("I · straight lane",   ShapeFamily.I,      () => WoolBoxEmitter.Emit(ShapeFamily.I, Bx(6, 12), Cw)),
        ("L · one bend",        ShapeFamily.L,      () => WoolBoxEmitter.Emit(ShapeFamily.L, Bx(12, 14), Cw)),
        ("Z · two bends",       ShapeFamily.Z,      () => WoolBoxEmitter.Emit(ShapeFamily.Z, Bx(12, 18), Cw)),
        ("Scythe · folded bay", ShapeFamily.Scythe, () => WoolBoxEmitter.Emit(ShapeFamily.Scythe, Bx(16, 14), Cw)),
        ("Clamp · two bars",    ShapeFamily.Clamp,  () => WoolBoxEmitter.Emit(ShapeFamily.Clamp, Bx(10, 14), Cw)),
        ("U · flush on crossbar", ShapeFamily.U,    () => WoolBoxEmitter.Emit(ShapeFamily.U, Bx(12, 16), Cw)),
        ("H · room-run stub",   ShapeFamily.H,      () => WoolBoxEmitter.Emit(ShapeFamily.H, Bx(12, 18), Cw)),
        ("Donut · ring (new)",  ShapeFamily.Donut,  () => WoolBoxEmitter.Emit(ShapeFamily.Donut, Bx(16, 12), Cw)),
        ("Donut · moved attachment", ShapeFamily.Donut, () => WoolBoxEmitter.Emit(ShapeFamily.Donut, Bx(16, 16), Cw, attachmentOffset: 4)),
        ("Donut · two attachments",  ShapeFamily.Donut, () => WoolBoxEmitter.Emit(ShapeFamily.Donut, Bx(16, 14), Cw, attachments: 2)),
        ("Donut · wool at ring corner", ShapeFamily.Donut, () => WoolBoxEmitter.Emit(ShapeFamily.Donut, Bx(16, 12), Cw, woolAtEnd: true)),
        ("Donut · wide attachment", ShapeFamily.Donut, () => WoolBoxEmitter.Emit(ShapeFamily.Donut, Bx(16, 16), Cw, attachmentWidth: 4)),
    });

Section("4-team pinwheel — rot_90",
    "one wedge fanned to four by 90° rotation about the centre",
    Take(s => s.C.T == 4, 8));
Section("Branch & fold approaches — U / H / scythe / clamp",
    "the escalated families: two legs off a crossbar, or a fold wrapping a bay",
    Take(s => Has(s.Fams, ShapeFamily.U, ShapeFamily.H, ShapeFamily.Scythe, ShapeFamily.Clamp), 12));
Section("Bent lanes — L / Z",
    "one- and two-bend corridors capping the wool",
    Take(s => Has(s.Fams, ShapeFamily.L, ShapeFamily.Z), 12));
Section("Straight-lane baseline — I",
    "the un-escalated approach: a single lane capping the wool inline",
    Take(s => true, 10));

// the scan surfaced this many donut arms in actual composed output (0 today — see the findings note)
string composedFamilies = string.Join(" · ", shots.SelectMany(s => s.Fams).GroupBy(f => f)
...
if (failures.Count > 0)
{
    ...
    failuresPanel = $"""
        <section class="panel panel--err">
          <h2 class="panel-title">Compose failures in the scan ({failures.Count})</h2>
          <p class="panel-body">Requests in the scan that threw rather than composing an acceptable plan.</p>
...
var outPath = Path.Combine("tools", "compose", "out", "box-gallery.html");
...
string Card(string id, Case c, string svg, int pieces, int stones, int holes, bool cut, List<ShapeFamily> fams)
{
    ...
    return $"""
            <article class="card">
              <div class="card-head"><span class="card-id">{Esc(id)}</span><span class="fam-badges">{badges}</span></div>
              <div class="svg-wrap">{svg}</div>
...
string Page(string sections, string failuresPanel)
{
    ...
    *{ box-sizing:border-box; }
    ...
    .grid--cat{ grid-template-columns:repeat(auto-fill, minmax(210px, 1fr)); gap:14px; }
    .card--cat .svg-wrap{ aspect-ratio:1/1; display:flex; align-items:center; justify-content:center; }
    .card--cat .svg-wrap svg.map{ max-height:100%; }
    ...
    .fam-badges{ display:flex; flex-wrap:wrap; gap:4px; margin-left:auto; }
    .fam-badge{ font-family:var(--mono); font-size:9.5px; letter-spacing:.06em; text-transform:uppercase;
      padding:2px 6px; border-radius:999px; border:1px solid var(--border); color:var(--text-muted);
      background:var(--bg-panel-2); font-weight:600; line-height:1.4; }
    .fam-badge--donut{ color:#0b3d24; background:var(--role-wool); border-color:var(--role-wool); }
    @media (prefers-color-scheme:dark){ .fam-badge--donut{ color:#eafff4; } }
    :root[data-theme="dark"] .fam-badge--donut{ color:#eafff4; }
    :root[data-theme="light"] .fam-badge--donut{ color:#0b3d24; }
    .fam-badge--u, .fam-badge--h, .fam-badge--scythe, .fam-badge--clamp{ color:var(--accent-light); border-color:color-mix(in srgb, var(--accent) 45%, var(--border)); }
    .fam-badge--l, .fam-badge--z{ color:var(--text-secondary); border-color:var(--border); }
    ...
    string note = $"""
        <section class="panel panel--note">
          <h2 class="panel-title">Finding — the donut is admitted, but the budget doesn't yet select it</h2>
          <p class="panel-body">G79 was the gate, not the trigger. The donut now clears the corner law and
          joins the fill menu (its shape is in the catalog above), but a wool box only takes a richer family
          when its budget <em>share</em> can hold that family's minimum box — and with the arm budget split
          across two wools, no share reaches the donut's (or U/H's) footprint at any tested player count.
          Across {shots.Count} composed plans the arms read <code>{Esc(composedFamilies)}</code> — the
          escalation past Z waits on the bigger single-arm boxes the partitioner allocates (G80/G63). The
          composed maps below are today's real output; the catalog above is the shape space G79 unlocked.</p>
        </section>
    """;
    ...
    <div class="wrap">
      <header class="top">
        ...
        pairwise corner rejection with a <em>cell-level</em> law — it scans the composed mask for a diagonal
    ...
    {sections}
      ...
      <footer>Static SVG · self-contained · fanned via PgmStudio.Geom.Symmetry — {cardCount} shapes + plans, cell = 5 blocks / proxy-cell.</footer>
    </div>
    """;
