...
Catalog("The approach families the box emitter fills",
    ...
    new (string, ShapeFamily, Func<EmittedApproach>)[]
    {
        ...
        ("Clamp · two legs",    ShapeFamily.Clamp,  () => WoolBoxEmitter.Emit(ShapeFamily.Clamp, Bx(10, 14), Cw)),
...
string BuildSvg(PlanModel plan, IReadOnlyList<Box>? boxes = null)
{
    ...
    if (boxes is not null)
        foreach (var bx in boxes)
        {
            ...
            double bx1 = bx.Rect.X * cell, bz1 = bx.Rect.Z * cell;
            double bx2 = (bx.Rect.X + bx.Rect.Width) * cell, bz2 = (bx.Rect.Z + bx.Rect.Height) * cell;
            for (int k = 0; k < order; k++)
            {
