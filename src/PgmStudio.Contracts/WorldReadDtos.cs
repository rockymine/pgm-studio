...
/// (<c>GET /api/map/{slug}/route</c>).</summary>
/// <param name="X">The cell's x.</param>
/// <param name="Z">The cell's z.</param>
/// <param name="Ground">The terrain's own recorded height, null over void.</param>
...
public sealed record RouteRunDto(string Material, int Cells);
...
public sealed record RouteGapDto(int FromX, int FromZ, int ToX, int ToZ, int Cells);

/// <summary>A drawn route walked end to end (<c>GET /api/map/{slug}/route?id=</c>) — the road as it was laid,
/// which <c>walk</c> cannot answer because that walks the way a player would choose.</summary>
...
/// <param name="Route">Whether the stroke declared itself a route — paint claims nothing and nothing stands
/// off it.</param>
...
public sealed record RouteReadDto(
    string Id, int Image, int Images, bool Route, IReadOnlyList<RouteStationDto> Stations, int Paved,
    int Rises, int Falls, int WorstStep, IReadOnlyList<string> Events,
    IReadOnlyList<RouteRunDto> Materials, int MaterialRuns, IReadOnlyList<RouteGapDto> Gaps);
