...
public static class EditRules
{
    ...
    /// <remarks>Create the region or filter first and wire the rule to it afterwards — <c>GET /api/map/{slug}/filters</c> lists the registry with what each filter is used by, and <c>GET /api/map/{slug}/regions</c> the regions. A filter that would reference itself wants the child it was meant to wrap instead.</remarks>
    ...
    /// <remarks>The edit is refused rather than the document: read the thing it names — <c>GET /api/map/{slug}/regions/tree</c> for a compound's children, <c>GET /api/map/{slug}/apply-rules</c> for the rule stack — and either give the edit the parts it is missing or apply it to something that can take it.</remarks>
