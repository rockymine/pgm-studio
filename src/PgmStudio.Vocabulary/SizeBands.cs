...
/// <summary>
/// The four sizes a CTW map is built at, named by the players-per-team band each serves. A map is not built
/// for one count — it works across a range — so the size a board is composed at is the band, and the player
/// count only selects it. <see cref="Centi"/> is the top of the ladder because the server it is built for
/// fields at most 32 a side, and a band above that is a size nobody plays.
///
...
public static class SizeBands
{
    ...
    /// <summary>32 and up.</summary>
    public const string Centi = "centi";

    /// <summary>The four, smallest first.</summary>
    public static readonly string[] All = [Nano, Micro, Milli, Centi];
    ...
    /// <see cref="Centi"/> being open, stated as the highest count a request may carry.</summary>
