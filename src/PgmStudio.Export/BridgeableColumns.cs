...
public static class BridgeableColumns
{
        ...
        IReadOnlyCollection<AnvilRegion.Chunk> chunks, Dictionary<string, object?> doc, int margin = 16)
    {
        ...
        var marks = FeatureExtractors.FloorMarks(chunks).Select(mark => (mark.WorldX, mark.WorldZ)).ToHashSet();
        return [.. Editability.Compute(doc, y0, (minX, minZ, maxX, maxZ), floorMarks: marks).BridgeableCells()];
    }
}
