...
/// A world, read.
///
/// <para><b>The boundary between reading a world and deriving from one.</b> Decoding Anvil chunks and
/// interpreting the NBT inside them is the world package's; what a monument or a core <em>is</em> is a
/// derivation, and derivations do not belong beside the format they happened to be read out of. This is
/// what crosses: blocks by position, and the three things in a world that carry an author's words or
/// intent rather than only material.</para>
///
/// <para>Nothing here knows the format it came from. A reading assembled by hand in a test is the same
/// input as one decoded from a region file, which is what lets the derivations be tested without a world.
/// </para>
/// </summary>
/// <param name="Blocks">Every non-air block in the region that was read, by position. Air is absent rather
/// than present as id 0, so a lookup that misses is a lookup at air.</param>
