...
public static class RoomEdges
{
    ...
    /// opposite end of it.</summary>
    public static bool AlongRunsRight(this RoomEdge edge) => edge is RoomEdge.NegZ or RoomEdge.PosX;
    ...
    /// the left hand (<see cref="AlongRunsRight"/>). A layout is worked out once in hand order and put back on
    /// the axis here; with <paramref name="width"/> 1 it turns a single step either way, and is its own
    /// inverse.</summary>
    public static int Handed(this RoomEdge edge, int lo, int hi, int fromLeft, int width = 1) =>
        edge.AlongRunsRight() ? fromLeft : lo + hi - (fromLeft + width - 1);
