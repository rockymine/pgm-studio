...
internal static class BlockPaletteData
{
    ...
    /// <summary>
    /// The sub-types a block <em>picker</em> offers, so andesite is a list entry rather than a number an
    /// author has to know.
    ///
    /// <para>Curated rather than derived from <see cref="Entries"/>. The catalog names every sub-type in the
    /// format because a render has to colour whatever a world contains, but a picker is a menu: enumerating
    /// the catalog would put mushroom-block faces and both halves of a double plant in a list of things to
    /// build with. This is the set worth offering — the stone, dirt, wood, sandstone, stone-brick, quartz and
    /// prismarine families, plus the plants whose whole point is their colour. Colours and names still come
    /// from the catalog, so an entry here cannot drift from what the render draws.</para>
    /// </summary>
    internal static readonly (int Id, int Data)[] PickerVariants =
    [
