...
{
    public override void Configure() { Get("/map/{slug}/regions"); Description(b => b.Refuses(404)); }
...
public sealed class CoverageEndpoint(MapRepository repo, MapReader reader, FeatureData feature, MapArtifactStore artifacts) : EndpointWithoutRequest<CoverageDto>
{
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        var traffic = Enumerable.Range(0, res.Height)
            .Select(iz => string.Concat(Enumerable.Range(0, res.Width)
                .Select(ix => TextGrid.Base36(res.Traffic[iz * res.Width + ix])))).ToList();
        await Send.OkAsync(new CoverageDto(
            new Bounds2dDto(res.MinX, res.MinZ, res.MinX + res.Width - 1, res.MinZ + res.Height - 1),
...
{
    public override void Configure() { Get("/map/{slug}/wool-availability"); Description(b => b.Refuses(404)); }
...
{
    public override void Configure() { Get("/map/{slug}/wool-suggestions"); Description(b => b.Refuses(404)); }
