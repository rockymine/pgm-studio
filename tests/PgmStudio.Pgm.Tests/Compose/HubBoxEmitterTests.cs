...
{
    private static readonly Box Box6x5 = new("hub", BoxKind.Hub, new(10, 20, 6, 5), 30);
    ...
    public async Task Rectangle_hub_fills_the_box_terminal_free_and_hub_labeled()
    {
        ...
        await Assert.That(hub.Pieces[0].Rect).IsEqualTo(new CellRect(10, 20, 6, 5));
    ...
    {
        var hub = HubBoxEmitter.Fill(new Box("hub", BoxKind.Hub, new(0, 0, 6, 5), 30), new CompoundRead(Compound.Rectangle), cw: 2)!;
    ...
    {
        var hub = HubBoxEmitter.Fill(new Box("hub", BoxKind.Hub, new(0, 0, 8, 5), 40), new CompoundRead(Compound.Rectangle),
            cw: 2)!;
    ...
    public async Task L_hub_is_a_spine_with_one_end_arm_its_side_wall_one_merged_run()
    {
        ...
        var hub = HubBoxEmitter.Fill(new Box("hub", BoxKind.Hub, new(0, 0, 8, 6), 48), new CompoundRead(Compound.SpineArms, 1), cw: 2)!;
    ...
    public async Task U_hub_opens_a_bay_two_offers_on_the_open_edge()
    {
        ...
        var hub = HubBoxEmitter.Fill(new Box("hub", BoxKind.Hub, new(0, 0, 8, 6), 48), new CompoundRead(Compound.SpineArms, 2), cw: 2)!;
    ...
    {
        var hub = HubBoxEmitter.Fill(new Box("hub", BoxKind.Hub, new(0, 0, 9, 9), 81), new CompoundRead(Compound.Ring), cw: 2)!;
    ...
    {
        var hub = HubBoxEmitter.Fill(new Box("hub", BoxKind.Hub, new(0, 0, 12, 8), 96), new CompoundRead(Compound.DoubleHole), cw: 2)!;
    ...
    public async Task Too_small_a_box_is_a_directed_null_not_a_throw()
    {
        ...
        await Assert.That(HubBoxEmitter.Fill(new Box("hub", BoxKind.Hub, new(0, 0, 3, 3), 9), new CompoundRead(Compound.Ring), cw: 2))
            .IsNull();
    }
    ...
    public async Task The_wide_holed_forms_fill_a_wide_box_and_fall_back_below_width_9()
    {
        ...
        var wide = new Box("hub", BoxKind.Hub, new(0, 0, 11, 6), 66);
    ...
    {
        var box = new Box("hub", BoxKind.Hub, new(0, 0, 12, 5), 60);
        ...
        await Assert.That(leg.Rect.Width).IsEqualTo(5);
        ...
        await Assert.That(plain.Pieces.Single(p => p.Slot == ApproachSlots.Leg).Rect.Width).IsEqualTo(Cw);
    }
    ...
    public async Task A_spine_too_short_for_a_notch_refuses_the_branch_form()
    {
        ...
        await Assert.That(HubBoxEmitter.Fill(
            new Box("hub", BoxKind.Hub, new(0, 0, 3, 5), 15), new CompoundRead(Compound.SpineArms, 1), Cw)).IsNull();
        await Assert.That(HubBoxEmitter.Fill(
            new Box("hub", BoxKind.Hub, new(0, 0, 5, 5), 25), new CompoundRead(Compound.SpineArms, 2), Cw)).IsNull();
        await Assert.That(HubBoxEmitter.Fill(
            new Box("hub", BoxKind.Hub, new(0, 0, 4, 5), 20), new CompoundRead(Compound.SpineArms, 1), Cw)).IsNotNull();
    }
