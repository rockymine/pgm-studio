...
/// <summary>
/// Zhang–Suen thinning + graph tracing. A thick bar thins to a line (two endpoints, no junction); a
/// T-shape thins to three lanes meeting at one junction (three endpoints, one junction).
/// </summary>
public sealed class SkeletonTests
{
    ...
    {
        var skel = Skeleton.Thin(Rect(0, 0, 40, 7));
    ...
    public async Task A_T_shape_has_three_lane_tips_and_one_junction()
    {
        ...
        var g = Skeleton.Trace(Skeleton.Thin(t));
    ...
    {
        var g = Skeleton.Trace(Skeleton.Thin([]));
