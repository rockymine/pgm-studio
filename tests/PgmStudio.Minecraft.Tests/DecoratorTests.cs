...
public sealed class DecoratorTests
{
    ...
    public async Task A_tree_does_not_root_where_a_building_already_stands()
    {
        ...
        var (world, top) = Plateau();
        var house = new HouseProp { Id = "h", Wings = [new AuthoredWing([[16, 16], [24, 24]])], Style = new HouseStyle { Door = DoorMaterial.Air } };
    ...
    public async Task A_second_building_does_not_stand_where_the_first_one_does()
    {
        ...
        var (world, top) = Plateau();
        var first = new HouseProp { Id = "h1", Wings = [new AuthoredWing([[10, 10], [18, 18]])], Style = new HouseStyle { Door = DoorMaterial.Air } };
        var second = new HouseProp { Id = "h2", Wings = [new AuthoredWing([[14, 14], [22, 22]])], Style = new HouseStyle { Door = DoorMaterial.Air } };
    ...
    public async Task A_trees_canopy_may_still_overhang_a_building_it_does_not_root_in()
    {
        ...
        var (world, top) = Plateau();
        var house = new HouseProp { Id = "h", Wings = [new AuthoredWing([[24, 16], [30, 24]])], Style = new HouseStyle { Door = DoorMaterial.Air } };
    ...
    public async Task A_building_is_decided_once_for_its_whole_orbit_not_once_per_image()
    {
        ...
        var tally = Decorator.Decorate(world, Context(top,
            [new HouseProp { Id = "h", Wings = [new AuthoredWing([[4, 4], [10, 8]])], Style = new HouseStyle { Door = DoorMaterial.Air } }],
            symmetry: "rot_180"));
