...
public sealed class WorldProvenanceTests
{
    ...
    public async Task An_unclaimed_column_answers_null()
    {
        ...
        await Assert.That(provenance.PassAt(5, 5)).IsNull();
    }
    ...
    {
        var provenance = new WorldProvenance();
        provenance.Claim(5, 5, ProvenancePass.Structure);
        await Assert.That(provenance.PassAt(5, 5)).IsEqualTo(ProvenancePass.Structure);
    }
    ...
    public async Task An_earlier_claim_after_a_later_one_still_wins_because_it_was_called_last()
    {
        ...
        provenance.Claim(5, 5, ProvenancePass.Structure);
        provenance.Claim(5, 5, ProvenancePass.Ground);
        await Assert.That(provenance.PassAt(5, 5)).IsEqualTo(ProvenancePass.Ground);
    }
    ...
    {
        var provenance = new WorldProvenance();
        provenance.ClaimRect(0, 0, 2, 2, ProvenancePass.Structure);

        await Assert.That(provenance.PassAt(0, 0)).IsEqualTo(ProvenancePass.Structure);
        await Assert.That(provenance.PassAt(2, 2)).IsEqualTo(ProvenancePass.Structure);   // inclusive corner
        await Assert.That(provenance.PassAt(1, 1)).IsEqualTo(ProvenancePass.Structure);
        await Assert.That(provenance.PassAt(3, 0)).IsNull();                               // one past the edge
    ...
    {
        var provenance = new WorldProvenance();
        provenance.Claim([(0, 0), (1, 0), (1, 1)], ProvenancePass.Ground);
        await Assert.That(provenance.Count).IsEqualTo(3);
        await Assert.That(provenance.PassAt(1, 1)).IsEqualTo(ProvenancePass.Ground);
    }
    ...
    {
        var provenance = new WorldProvenance();
        provenance.Claim(5, 5, ProvenancePass.Structure, new StampId("house", "d-h1", 0));
    ...
    public async Task A_later_claim_overwrites_an_earlier_claims_owner_too()
    {
        var provenance = new WorldProvenance();
        provenance.Claim(5, 5, ProvenancePass.Structure, new StampId("house", "d-h1", 0));
        provenance.Claim(5, 5, ProvenancePass.Structure, new StampId("house", "d-h2", 0));
    ...
    {
        var provenance = new WorldProvenance();
        provenance.ClaimRect(0, 0, 2, 2, ProvenancePass.Structure, new StampId("wool", "0", 0));
    ...
    {
        var provenance = new WorldProvenance();
        provenance.Claim([(0, 0), (1, 0)], ProvenancePass.Structure, new StampId("house", "d-h1", 0));
