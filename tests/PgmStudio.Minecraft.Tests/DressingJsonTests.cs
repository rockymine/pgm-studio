...
public sealed class DressingJsonTests
{
    ...
    /// <summary>The biome patches ride in the same document as the placements and round-trip with them. A
    /// patch is a shape drawn on the same canvas, so a document read and written back has to keep its outline
    /// point for point — the generated equality compares those by reference.</summary>
    [Test]
    public async Task A_biome_patch_survives_the_round_trip()
    {
        var doc = new DressingDoc
        {
            Biomes =
            [
                new BiomePatch
                {
                    Id = "biome-1",
                    Points = [[0, 0], [8, 0], [8, 8]],
                    Field = new CellBiome(3, CellSize: 16, Jitter: 70, Palette: [Biome.Desert, Biome.Taiga]),
                },
            ],
        };

        var read = DressingJson.Deserialize(DressingJson.Serialize(doc));

        await Assert.That(read.Biomes.Count).IsEqualTo(1);
        await Assert.That(read.Biomes[0]).IsEqualTo(doc.Biomes[0]);
    }

    /// <summary>A document that draws a patch and places nothing is a document, not an empty one — the
    /// props key is what a map that never opened the phase lacks, and the patches are the other half.</summary>
    [Test]
    public async Task A_document_with_patches_and_no_props_reads_its_patches()
    {
        var read = DressingJson.Deserialize(
            """{"biomes":[{"id":"a","points":[[0,0],[4,0],[4,4]],"field":{"kind":"solid","id":2}}]}""");

        await Assert.That(read.Props).IsEmpty();
        await Assert.That(read.Biomes.Count).IsEqualTo(1);
    }

    /// <summary>A patch that will not read is refused by name and position, the way a prop is — a hand-edited
    /// blob with one bad field must not cost the whole map its dressing silently.</summary>
    [Test]
    public async Task A_patch_that_will_not_read_is_refused_by_name()
    {
        var fault = Assert.Throws<DressingParseException>(
            () => DressingJson.Deserialize("""{"props":[],"biomes":[{"id":"a","field":{"kind":"cabbage"}}]}"""));

        await Assert.That(fault!.Subject).Contains("biome patch 'a'");
    }
}
