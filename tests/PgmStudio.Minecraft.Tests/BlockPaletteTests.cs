...
public sealed class BlockPaletteTests
{
    ...
    {
        await Assert.That(BlockPalette.Hex(1, 0)).IsEqualTo("#707070");    // Stone (112,112,112)
        await Assert.That(BlockPalette.Hex(2, -1)).IsEqualTo("#7fb238");   // Grass (127,178,56)
    }

    [Test]
    public async Task Stained_block_uses_its_damage_as_a_sub_colour()
    {
        await Assert.That(BlockPalette.Hex(35, 14)).IsEqualTo("#993333");  // Red wool (153,51,51)
        await Assert.That(BlockPalette.Hex(35, 0)).IsEqualTo("#dddddd");   // White wool (221,221,221)
    }
    ...
    public async Task Any_data_falls_back_to_the_minus_one_entry()
    {
        ...
        await Assert.That(BlockPalette.Hex(35, -1)).IsEqualTo("#dddddd");
    }

    [Test]
    public async Task Names_format_known_and_stained_blocks()
    {
        await Assert.That(BlockPalette.Name(1, 0)).IsEqualTo("Stone");
    ...
    {
        var a = BlockPalette.Hex(250, 3);
        var b = BlockPalette.Hex(250, 3);
