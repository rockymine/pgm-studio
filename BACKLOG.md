...
- [ ] **B209 — The showcase is a generated map, and it is generated by a second world builder.**
  `tools/PgmStudio.PatternMap` (597 lines over three files) exists to prototype patterns and to be a map you
  can walk. It is **already a normal generated map** — two teams, asymmetric, islands, a plan-free layout —
  and almost all of it is now sayable as a `mapgen` spec, which it was not when it was written. What it should
  turn into is a spec plus one capability the system genuinely lacks.

  **The inventory, in four parts.**

  *Sayable as spec JSON today, verbatim.* `MapSpec.Layout` and `MapSpec.Intent` carry a `SketchLayout` /
  `MapIntent` fragment through unchanged, which covers every document the tool builds: the 27 named themes
  (already `TerrainThemeJson`-serialised in the code), the 27 shapes and their islands, the 100 tree props and
  10 house props (`layout.dressing.props` — which `MapSpec` names as exactly where a prop goes), the bound oak
  room shell (`layout.roomStyles`, or `room_shell` by preset name), and the whole intent — two teams, two
  spawns, two wools, the room floors and the entrance redstone. `slug`/`name`/`objective`/`out_dir` are spec
  fields. The `--render` flag added on 2026-08-15 is `stages: true`, which mapgen has had all along and emits
  eight named images for rather than one.

  *The capability that is actually missing.* `Shapes.Origin(index)` — two lines placing island *i* on a grid
  of five columns at a 56-block pitch. Every shape, every tree, every building and every room piece derives
  its position from it. A spec cannot say "lay these out on a grid", and nothing else in the studio can
  either. This is the one thing to build, and it is a **layout** capability rather than a plan one: a plateau
  here is a circle, an octagon, a cross or a wedge, and a plan piece is a rectangle — so it emits a
  `SketchLayout` beside `compose` and `plan`, not a `PlanModel` through them.

  *Duplicated with mapgen already, and this is the sharp part.* `tools/mapgen/Materials.cs` and
  `tools/PgmStudio.PatternMap/Plateaus.cs` both carry the same palette vocabulary, and not merely the same
  idea — the same characters:

  | in both files | |
  |---|---|
  | `TopFaced(id, data) => (id, data) is (2, 0) or (3, 2) or (110, 0)` | identical |
  | `Family(name, take)` · `One(name, index)` | same shape, one with the error message |
  | `Grass` = `LayeredMaterial(grass 1, dirt 2)` | identical |
  | `new VoronoiMaterial(1, 9, …, Rise: 8)` | identical constants |
  | `new CellMaterial(2, 10, 55, 4, …, Rise: 8)` | identical constants |
  | `new NoiseMaterial(3, 14, 3, …, Rise: 8)` | identical constants |
  | `new TurbulenceMaterial(4, 14, 3, …, Rise: 8)` | identical constants |
  | `new ElectricMaterial(5, 16, 3, …, Rise: 8)` | identical constants |

  The docstring explaining why grass, podzol and mycelium come out of a family is written twice as well, in
  near-identical prose — the tell `CLAUDE.md` names. **None of it is in `src/`**, so the studio itself cannot
  reach the answer either tool uses; two `tools/` copies is the worst arrangement of the three. Which block a
  family holds and what seed a named pattern is laid at is studio knowledge, and belongs beside
  `TerrainPalette` in `PgmStudio.Minecraft`.

  *What only the showcase has, and must survive.* It **sweeps the catalogues**: one plateau per pattern, one
  per house preset (10), one tree per species (6) and per wood (6), over 19 palette families — plus a shape
  vocabulary chosen to test corner detection (rectangle, disc, octagon at the 45° threshold, cross with inner
  and outer corners, wedge at 80°). A frozen JSON file cannot notice that an eleventh house preset was added,
  ...
  **The shape of the fix.** The palette vocabulary moves to `src/` and both tools read it. A grid-of-islands
  ...
  `SketchWorldBuilder` → `AnvilRegionWriter` → `WorldProvenanceFile` → `LevelDatWriter` → `map.xml` sequence
  (`B208`, the same seven steps written twice). It keeps its name only if it grows into what it actually is —
  a **library map**: every pattern, every tree, every house, or all three at once, asked for by name.

  *inventoried 2026-08-15 while adding the inward-axis plateaus · `tools/PgmStudio.PatternMap/*` ·
  `tools/mapgen/Materials.cs` · `MapSpec.Layout`/`Intent`.*
