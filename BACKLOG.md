...
### The edge: one envelope, and the four the endpoints answer instead

`docs/refusals.md` opens by saying every gate in the studio answers in one shape, and the gates do. The
**edge** does not: 52 sites write the envelope and 122 write something else, so a caller cannot parse one
refusal and be done. The four entries below are that split by cause, and the fifth is the helper the edge
already has and copies anyway.

- [ ] **RP5 — Forty-eight refusal sites at the edge answer a bare `{error}`.** No rule id, no field, no
  `findings`, so nothing an agent can act on and nothing `GET /rules` can explain. Three shapes: 24 write
  `new { error = … }` or `["error"]` directly (`ComposeEndpoints` 197/219/243/263, `SketchEndpoints`
  276/330/380/439/485/498, `HousePartLibraryEndpoints` 139/226/294 `"in use"`, `MapPlanEndpoints` 103/132,
  `TerrainPreviewEndpoints` 307, `WriteEndpoints` 28); 19 go through `ImportEndpoints.Fail`; 5 answer
  FastEndpoints' own `{errors}` through `AddError` + `Send.ErrorsAsync`. Convert each to
  `Refusals.UnreadableAsync`/`WriteAsync` with the rule it is — most are `RQ1`, the three `"in use"` are a
  409 naming what still binds, and `"stored plan is unreadable"` (×5) is the studio's own artifact failing to
  parse rather than the caller's document, which `RQ2` already covers.

  *`grep -c "Refusals\.\(WriteAsync\|StopAsync\|UnreadableAsync\|Of\)" src/PgmStudio.Api/Endpoints/*.cs`
  = 52 against 24 + 19 + 5 the other way.*

- [ ] **TE1 — The Edit tool answers every refusal as `{error: "…"}`, ~70 of them.**
  `EditException(status, message)` is thrown at 74 sites across `Pgm/Editing`, and `WriteSupport.RunEditAsync`
  (`Api/Endpoints/WriteEndpoints.cs:23`) turns each into `new Dict { ["error"] = ex.Message }` with the
  exception's status — for the 36 endpoints that route through it, which is the whole Edit surface. So the
  tool with the most refusals in the studio is the one with no rule ids: a region id already in use, a filter
  something still references, a coordinate that is not an integer all arrive as prose. Give the family its
  own ids (`ED*` in `docs/refusals.md`), have `EditException` carry a `Finding` rather than a string, and
  answer `Refusals.Of` from `RunEditAsync`. Its unknown-map 404 carries `{error: "map not found"}` where the
  other 42 endpoints answer an empty 404; that goes with it. `docs/tools/edit.md` names the statuses it
  refuses with and never the body, so the doc gains the shape in the same commit.

- [ ] **RP6 — Five catch-alls dress a fault nobody read as the caller's mistake.** `catch { await
  Send.ResponseAsync(new { error = "could not paint layout" }, 400, ct); }` and its four siblings
  (`SketchEndpoints` 276/330/380/439, `TerrainPreviewEndpoints` 307) swallow **every** exception a build or a
  solve can raise, log none of it, and answer 400 — which is the one thing `Program.cs`'s unhandled-fault
  middleware exists to prevent, since dressing a defect as a bad request sends an author hunting a mistake
  they did not make. Narrow each to the document faults by type, the way `PlanColumnsEndpoint` already does
  (`catch (Exception ex) when (ex is ArgumentException or InvalidOperationException or … or JsonException)`),
  and let anything else reach the middleware as `RQ2` with its trace logged.

- [ ] **TC3 — `box=` is parsed by two copies of one function with two behaviours.** `TryParseBox` is
  duplicated verbatim in `CoreEndpoints.cs:69` and `MonumentEndpoints.cs:54`. Monument refuses a malformed
  one (`RQ1`, `field: "box"`, naming the form); core-suggestions ignores the failed parse and answers every
  casing it has, unfiltered, with a 200 — so a mistyped box reads as "this volume holds them all". One parser
  at the edge answering `BlockBox?`, and both routes refusing what it cannot read. The parameter is optional
  on one route and required on the other by design (`docs/tools/configure.md`), and that stays; what changes
  is that *stated and unreadable* stops meaning *not stated*.

  *`GET /map/{slug}/core-suggestions?box=garbage` → 200 with the list; `GET …/monument-suggestions?box=garbage`
  → 400 `RQ1`.*

- [ ] **TL1 — One fault, four bodies, on the surface a helper already owns.** `?format=png&view=…` is read by
  `PngAnswer` (`TerrainPreviewEndpoints.cs:71`), but the refusal for a view name it does not have is written
  by the callers: `AddError("view must be plan or section") + Send.ErrorsAsync(400)` three times with three
  wordings (`TerrainPreviewEndpoints` 105/135/188) and `new { errors = new[] { RoomStylePreview.PngViews } }`
  once (`RoomStyleLibraryEndpoints.cs:34`). `PngAnswer.View` is what reads the name, so it is what should
  refuse it — `RQ1`, `field: "view"`, the names it does have in the sentence — and the four callers pass their
  own list rather than their own envelope.

- [ ] **RP7 — The edge copies the two helpers it already has.** `RawBody.ReadAsync`
  (`TerrainPreviewEndpoints.cs:58`) is called 4 times and hand-rolled as the same two lines 19 times;
  `ImportEndpoints` declares `Fail` twice, identically, at lines 100 and 261 for its two endpoint classes.
  Mechanical, and worth doing while the envelope pass above is rewriting the same call sites — a body read by
  hand is also the reason `RequiredFields` cannot see it, so the two are one pass.
