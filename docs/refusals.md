...
| `PT*` | a terrain theme's own materials — `PT1` a surfacing block painted below the course it surfaces, `PT2` a band, stop or side stated with no material in it, `PT3` a sampled pattern whose period is finer than the blocks it paints (the floor is 2 blocks; a drawn pattern — a checker's square, a wall run's stripe — is not asked) | `Minecraft/Painting/TerrainThemeValidation.cs` → `TerrainThemeRules` |
