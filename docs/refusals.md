...
| `DC*` · `OB*` | destroyables, cores, goal placement, gamemodes. `OB24` is the one that needs the built world rather than the plan: two goals stamped into the same blocks, which the plan cannot see because it reads two placements at two coordinates and the volumes around them are the stamper's. Two are **complaints on a built world**: `DC3` a goal built in a material its own size is wrong for, `OB23` a goal topping out over the build ceiling — the world is built and the goal stands, which is what makes them complaints rather than declines. `OB19` is the decline beside them: the prop the pass turned away is not in the world at all. `OB25` is a third complaint on a built world and the only one about a field rather than a shape: a wool monument's location is filled from the air cell the capturing team's spawn structure stamped, so an authored one is replaced — three write paths take a location and every read answers it, which is what makes the replacement worth a sentence rather than a silence. On a map the studio did not build there is no stamp and the stated location is authoritative. `OB26` is a fourth, and is about the document rather than the world: a destroy map with no mode ladder, or with one none of its goals opted into, has no way to end — PGM lets the owner repair a destroyable unless the map says otherwise, a core has no `repairable` at all, and the obsidian an attacker drops is what the defender rebuilds with, so the material changing on a clock is the map's only answer. A complaint on `PL3`'s precedent: the map builds and loads, and how long a match may run is the author's | `Domain/ObjectiveRules.cs` |
| `WX*` | room frames — the footprint and where it sits on its piece, the pad, the doors, the iron, and the shell's height against the build ceiling. `WX3`/`WX4` are the pad's own and the fit that raises them is `Domain/SpawnPad.cs`, which a caller with no room reaches too; the ids stay with the resolver that reports them | `Domain/RoomFrames.cs` → `RoomFrameRules` |
| `HS*` | a house style's own materials | `Minecraft/Houses/HouseStyleValidation.cs` → `HouseStyleRules` |
| `PT*` | a terrain theme's own materials — `PT1` a surfacing block painted below the course it surfaces, `PT2` a band, stop or side stated with no material in it, `PT3` a sampled pattern whose period is finer than the blocks it paints (the floor is 2 blocks; a drawn pattern — a checker's square, a wall run's stripe — is not asked), `PT4` a sampled field on a face with no `rise`, so the face comes out in vertical stripes — asked of both tall buckets, the wall where it paints and the fill always, every band of a stack whatever its thickness. `PT5` is the one that needs a board rather than a theme: a team tint is one colour per canonical island, so an island more than one team's spawns stand on wears one team's colour everywhere, and on a board whose ground is a single landmass that is the whole map. A **complaint on a built world**, raised where the painter resolves its themes (`Export/WorldBuilder`), asked of the themes that actually resolve on the shared island's cells rather than of the registry | `Minecraft/Painting/TerrainThemeValidation.cs` → `TerrainThemeRules` |
...
| `DR-*` | the dressing pass's own — `DR-DOC` a document that will not parse, `DR-ROAD` a prop resting nearer to the road than its kind's standoff (the numbers live on `PlacedProp.RouteStandoff`), `DR-PASS` a building leaving under eight blocks of passage along the whole run of any side, counted from what the building stamps rather than from its walls, since a roof oversails its wall by at least one block; every side is asked, because the lane a building stands in is the ground players arrive on rather than a way round it, and a side the ground stops flush against is the map's edge or a hole — a building may stand against one of those but not against two facing each other; asked round the **group** of buildings a building stands in — every building within a passage of it, transitively — since a player walks round a village rather than between every pair of its houses, which leaves the floor inside one to `DR-CLAIM`'s ring. A complaint: the building is in the world, standing where it was put, since where a building stands is something an author moves. `DR-SIZE` a building whose box is under 5×5 blocks, `DR-KEEP` a prop resting on ground the map keeps clear (a spawn, a wool room, a stated structure, a built column, a sketch shape marked `keepClear`, a door's approach), `DR-CLAIM` a prop resting on ground something already standing holds on the same layer (a building holds what it stamps plus a block of ring beyond it; a **tree holds the disc its crown covers**, at `Decorator.CanopyRadius` — its own farthest leaf — rather than the columns its blocks happen to fill, since a crown is leaves with gaps and a trunk seated in one gap grows its crown through the other tree's; the claim book is keyed on the layer, so a prop over a channel it does not share ground with is not this), `DR-CUT` a prop the clip cut in two — a prop is asked about at its **feet** and written wherever the rest of it meets air, so one that seats clear of a building may still reach into it, and where the obstruction takes out the blocks joining a limb to the body the far half of that limb is written into open air with nothing holding it; counted against the prop as it would have stood on open ground, since a crown gathered at the branch tips is several pieces to begin with, and raised off what was **severed** rather than how much was lost — a rock flattened along a wall loses a third of itself and severs none, while a twenty-course tree eight blocks clear of a wall places 429 of its 430 blocks and leaves a limb floating. A second arm reads the share the ground **blocked**, which is the same fault from underneath: a prop seats on the lowest column its feet cover, so on stepped ground its body is written into terrain standing over it, the remnant that lands rests on real ground and severs nothing, and past half the body blocked what stands there is not the prop that was placed. A complaint: the prop is in the world as the clip left it. `DR-FACE` a block of a copied body naming a face it has nothing to cling to — a copied body is written with the data it was cut with, so a block whose data *is* a direction has to point at something the body holds; a vine side naming air is a curtain on nothing, and a pair of opposite sides is that fault seen from the front. Asked once per body, not per placement. `DR-COPY` a tree recipe saved as `copied` with no cut recorded — a copied tree is one cut out of a world and the cutter is what records where, so a body with no cut behind it is refused where the recipe is saved (`POST`/`PUT /tree-styles`, 400, `docs/tools/library.md`). `DR-BANK` a body of water that dug a shaft rather than filling a hollow — its line is one plane and every bed column standing over it is emptied down to it, so `depth` bounds the bed and nothing bounds the bank, and a pond drawn across a slope comes out a straight-sided pit as deep as the ground falls; measured against the author's own stated depth. `DR-DRY` water standing against a hole in its own basin — the bed a pool carves and the hollow it sits in are two statements about one lake, so where the hollow reaches further the extra is dug and never filled and the water stands against open air; air is a fault only where the board **drew** ground there, since a pool reaching the board's own edge meets the void and a wall of water at the world's rim is what a coast is (the author's ruling). `DR-LAYER` a prop naming a layer the board has no ground on — declined rather than seated on the top surface, which is the layer the author was saying they did not mean; `DR-SITE` a prop with no ground to rest on — a building is held to every cell of its footprint, and the finding names the first bare column; `DR-STEEP` a boulder standing on a face rather than on ground — an erratic is a mass left where the ice dropped it and a rock pinned to a steep hillside reads as neither, the slope being the feature there already; the angle is `SurfaceGradient.Degrees` over the surface the pass seats on, and the angle it is judged against is the board's own, since a surface graded by `BandAxis.Slope` has already said where its cliff begins (the band covering the steepest ground) and a surface grading by nothing is read at `Materials.DefaultCliffAngle`. Ground the middle band paints is still ground. A complaint: the world is built and the rock is in it. `DR-ROOT` a tree standing on ground nothing grows out of — a tree is a thing that grew where it is, so the block under its trunk is the one that says so, and on stone, gravel, clay or paving it reads as a model set down rather than as a wood; grass and the three dirts by `DressingPalette.RootsInto`, which is stricter than the flora overlay's own `SoilShare` because a tuft of grass in a shingle is ordinary and a trunk out of one is not (the author's ruling). The fix is usually the paint rather than the position, and `POST …/sketch/seats?kind=tree` refuses every cell without soil under this same id. A complaint: the world is built and the tree is in it; `DR-TONE` a boulder cut from nothing but the tone families of the ground under it — a rock reads as a rock by not being made of the field it sits in, so the question is asked in `TerrainPalette` families rather than in blocks (a sandstone rock on sand is two blocks and one tone) and what fires is *built wholly from* the field, a rock keeping one family the ground lacks being a rock however much else it shares; judged against the ground it can **rest** on, a slope stack's steep faces being cliff nothing stands on. A complaint, and the one `DR-*` raised before the world is built rather than as a decline on one; `DR-SLOPE` a building whose footprint is not level enough to stand on — a building seats on its lowest column and the ground over that floor is carved out of it, so a site rising by as much as the building itself stands builds a house whose uphill side is under the ground beside it, roof and all; `DR-DIG` a building that dug its site out of the ground more than three blocks deep at some column (`DressingRules.SettleDepth`; up to three is a house settling into a slope, past it the house is probably submerged — the author's number) — it seats one course under its lowest column and clears every other column down to that floor, so a footprint whose lowest cell sits in a pit takes the pit's depth out of the whole plan; the finding carries the floor's course, the deepest carve and its column, the columns carved and the blocks removed. A complaint.. All but `DR-DOC` and `DR-COPY` are raised **on a built world**, and the severity says what became of the prop: a decline is a prop the world was built without, a complaint one that is in it and worth looking at | `Minecraft/Dressing/DressingJson.cs` · `Minecraft/Dressing/GroundClaims.cs` → `DressingRules` |
| `SH*` | a shop's references to the rest of the document, which nothing inside a `<shops>` block can check for itself. `SH1` is the only one: a keeper's `shop`, a keeper's `region` or an icon's `action` naming something the document does not define. PGM resolves all three at load — the first by name in `ShopModule.parse`, the other two as feature references whose `resolve()` throws — and refuses the whole map on any that fails, so this **refuses** beside `EX2` rather than complaining. Asked only of an intent-authored board, whose menus are exactly what its intent stated; an imported map may take its menus from an `<include>` the parser reads without splicing, which fifteen corpus boards do | `Domain/ShopRules.cs` → `ShopRules` |
...
| `RL*` | a solved relief read two ways — against what its group says it is (`RL1` ground that is not the landform stated, `RL2` elevation that was never graded, `RL5` elevation graded everywhere with nowhere left to stand), and against the statements that made it (`RL3` two marks meeting on a step, `RL4` a mark that pinned nothing, `RL6` a push whose skirt and crown climb at rates far enough apart to step at its own outline). Complaints throughout: a relief is authored ground, and the studio measures it rather than overruling it | `Analysis/Playability/ReliefRules.cs` → `ReliefRules` |
...
| `DR-PASS` | does this building, or the group it stands in, leave eight blocks to get past it on every side that is not an edge? | one prop on that ground |
...
**A finer grain exists only because a coarser one does.** `DR-PASS`'s eight blocks are meaningful because the
...
`SK10`, `SK11`, `SK13`, `SK14`, `SK15`, `SK16`, `SK23`) and `DR-TONE` are read off the rasterized spans, and
answering them walks every column of the board, so a write names them on `Pgm-Unwalked` instead and
`GET /map/{slug}/findings` answers them on demand.
