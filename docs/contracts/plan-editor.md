...
1. **PL-schema** — `PlanModel` + parser + `PlanValidator` (errors + rule lint), pure, tested.
2. **PL-compile** — `PlanCompiler` + step terraces + the three seed plans with golden-file
   regression against the existing seed pairs.
3. **PL-editor** — the canvas page: grid, rects, roles, heights, markers, mirror ghost,
   import/export, autosave.
4. **PL-live** — derived-structure overlays + live lint panel.
5. **PL-loop** — compile preview + Create-draft/export wiring (the walk-test loop).
6. **PL-corpus** — the author burns down the seed shopping list; each new seed lands in
   `tools/seeds/` as `*.plan.json` (+ its compiled pair, regenerated by CI/test, not by hand).

Milestone task filing on the board happens when this merges (new tasks land in `BACKLOG.md` /
`TODO.md` per the board rules); ids proposed as a `PL` block or continuing the `G` series.
