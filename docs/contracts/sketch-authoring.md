...
`{"props": [...]}`, each prop a `kind`-discriminated object — a `path` (an open route plus a half-width, a
...
| `canvas/sketch-layout-canvas.js` | **`canvas/sketch-canvas.js`** | extends `CanvasBase`; **world coords ARE svg base coords** (identity transform, no `buildTransform`) — keep that distinction from WorldCanvas. |
| `canvas/sketch-draw-controller.js` | **`controllers/sketch-draw-controller.js`** | rect (drag) / circle (2-click) / polygon (click+close) / **lasso** (drag-trace). Same controller contract as `world-draw-controller` (`onMouseDown→bool`, …). |
