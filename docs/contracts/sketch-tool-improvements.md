...
## 8. Shape library — drag-in primitives (S8, retired)

The palette of pre-defined primitives — regular n-gons, the polyomino set (L · U · T · I-bar · scythe ·
cross · line-with-branch) and the composite hole-square — shipped and has since been removed. The plan
editor supersedes it: a board's rectilinear structure is laid out there as proxy-cell pieces and surfaced
into the sketch, which is the same clean topology the palette existed to hand over, arrived at from the
intent rather than from a shape name. What remained was a twelve-tile grid competing with the island tree
for the sidebar, and an arm-then-click mode with no toolbar state of its own.

Two pieces of it outlived the palette and are described where they belong:

- **Whole-shape body-drag move** was bundled here originally, then promoted to **`CV10`** as a
  cross-canvas affordance: one seam in `CanvasBase` (which already owns the drag lifecycle) plus per-canvas
  `_hitMovable` / `_moveBy` / `_commitMove` hooks. Shipped for Edit and Sketch alike.
- **Snap-alignment guides** — shapes snapping to other shapes' edges and centres while dragging, with Alt
  as the bypass — shipped as the Snap toggle on the sketch sub-bar.
