...
| `ShapeEmitter` | The **pure family geometry** — fills a `W×H` box with a base family at a corridor width, returning slot-typed rectangles in a canonical frame. No roles, no ids, no plan types. |
...
| `EmittedShape` | A **pure emission**: box-local slot-typed terrain rectangles, the terminal rectangle, the marker offset, and the shape's vacancies. What `ShapeEmitter` returns. |
...
| `IsolationCut` · `CutResult` | The **isolation-cut** fragmentation move: sever one marker piece behind a build bridge (a team's own `spawn↔wool` cut). |
