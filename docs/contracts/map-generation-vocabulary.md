...
| `NegativeSpace` · `ClassifiedEdge` | One connected negative space read off a body (its kind, cells, wall count, parts), and one maximal straight boundary run classified on **two independent axes**: the space it **faces**, and its **owner** — `Terminal` marks a run on the terminal room's own wall (a fact; the never-attach verdict over it is the docking gate's rule). Runs split where ownership changes, so a room capping a lane leaves the line part free, part sealed. The free offerable surface = `Open` ∧ not `Terminal`. |
