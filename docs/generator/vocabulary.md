...
| `PlanNav` · `PlanPoi` | The **plan as ground someone walks**: the fanned board as a navigable cell set — `Ground` (every generating piece), `Bridge` (a build zone's cells, void crossed by building), `Lane` (a water lane, drawn and not walked before it opens), the enclosed `Holes`, and every marker as a `PlanPoi`. One tier above the piece graph and one below the built world: `ContactGraph` says whether two rectangles touch and `FannedGraph` whether one reaches another, and neither can say how far, how wide, or how many ways. `Walkable()` hands those sets to the shared `Walk` as the plan tier's `WalkGround`. |
