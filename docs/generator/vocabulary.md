...
| `Finding` · `Severity` | **One thing wrong with something an author wrote**, in the one shape every gate in the studio answers in (`docs/refusals.md`): a stable rule id, a sentence carrying the measured numbers, and what it is about — a document field, the ids it indicts, or neither. A refusal stops the work and a complaint rides along with it. Lives in `Domain`, the lowest project every gate can reach. |
