...
| `SizeBands` | The **five sizes a CTW map is built at**, in `PgmStudio.Vocabulary` as words rather than an enum since the composer keys its budget on one, the compose surface answers which a board was built to, and the client offers them as the sizes a caller may ask for: `nano` 6–13 players a team · `micro` 14–21 · `milli` 22–31 · `centi` 32–47 · `hecto` 48+. `Of(players)` is the one statement of the ladder and `Rank` orders by size rather than alphabetically. A map is built for a range, not a count, so the band is what the land budget and the corridor width read (G8, G2). |
