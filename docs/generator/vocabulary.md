...
| `StructureSummary` · `StructureNames` · `DockArrangement` | The **unit's structural read as one small fact**: the sorted wool approach families (`ShapeClassifier.Classify` per wool box), the hub body form, the frontline form or `none` (`ClassifyBody` per hub/frontline box), and the **dock arrangement** — which hub side the spawn and each wool seat on, in the unit's own compass (`DockArrangement.Of`, reading the `BoxPartition` joints against the spawn's facing). The arrangement classifies **canonical** (spawn back, wools flanking — the arrangement built maps converge on; roughly half the spawn-distance imbalance) against **lopsided** (spawn lateral, a wool on the back) — `match-flow.md` §3.3. Read off the labeled grown unit (box + slot), never a finished map, so it works uniformly on a bare unit and needs no labels stored back. `StructureNames` maps it to lowercase display/filter tokens (`donut`, `ring`, `twin`, `bar`, `canonical`, `none`…); `Canonical()` is the stable bucket key (`wools:donut,l\|hub:ring\|front:none\|seat:canonical`). The **filter fact** for the browse sieve (G117) and the bucket key for verdicts (G118) / duels (G120). |
