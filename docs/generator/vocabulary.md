...
| `PlanGlobals` · `PlanMeta` | The plan's **frame**: cell size, symmetry, player cap, base surface height and `maxBuildHeight` (globals); name/metadata (meta). The build ceiling is **stated** — it was `headroom`, a slack over `surface`, which computed the cap from the flat nominal world the relief solve abandons (B105). |
