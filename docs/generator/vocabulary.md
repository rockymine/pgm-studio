...
**§9** is the *code map* (which file); **this doc names the types that embody the ideas**, in the order the
...
The pipeline these are ordered along (from `model.md` §1):
...
The budget is **two currencies that must both balance** (§2): **land** (walkable terrain area, set by the
...
Before any piece is filled, the budget draws a **coarse partition of typed boxes** (§6). A box is a *bounding
...
An **interface** is always a shared **edge interval** — a position and a width — where two boxes meet (§6);
never a point. The **interface width** is the master variable of generation (§2): it sets connectivity,
...
| `SizeBands` | The **four sizes a CTW map is built at**, in `PgmStudio.Vocabulary` as words rather than an enum since the composer keys its budget on one, the compose surface answers which a board was built to, and the client offers them as the sizes a caller may ask for: `nano` 6–13 players a team · `micro` 14–21 · `milli` 22–31 · `centi` 32 and up. `Of(players)` is the one statement of the ladder and `Rank` orders by size rather than alphabetically; `Players(Centi).High` is the ceiling a request is clamped to, since centi is the top. A map is built for a range, not a count, so the band is what the land budget and the corridor width read (G8, G2). |
| `NegativeSpaceKinds` | The **wall-count escalation of negative space**, in `PgmStudio.Vocabulary` as words rather than an enum, since the composer, the plan lint and the wire all spell them: `notch` (wrapped by two edges — the L's corner) · `bay` (three — the staple's recess, the hook's bay) · `hole` (enclosed — the ring's void) · `open` (at most one wall — plain outside, not a feature). `Of(walls, enclosed)` is the one statement of the escalation and `Rank` orders by it rather than alphabetically. The classes that decide which voids are publishable, which edges remain offerable, and which gaps `WL12` measures. |
| `NegativeSpace` · `ClassifiedEdge` | One connected negative space read off a body or off a board's own terrain (its kind, cells, wall count, parts, `Crossings`, and its **own compound `Form`** — the void read as a body: the uneven branch's bay is a `SpineArms(2)`), and one maximal straight boundary run classified on **three independent axes**: the space it **faces**, its **owner** (`Terminal` — the room's own wall), and the **guard** (`Guarded` — inside the room's clearance margin). Runs split where any axis changes; the free offerable surface = `Open` ∧ ¬`Terminal` ∧ ¬`Guarded`. Ownership/guard are facts; the never-attach verdicts over them are the docking gate's rules. |
...
| `SpaceCrossing` | One straight line over a negative space that terrain closes at **both** ends — its length in cells, the owner walling each end, and where it starts. This is what a player crosses: a run open at one end is a way out of the space rather than a gap over it, so only closed runs are measured. `BoardStructure.Spaces` carries them for an authored plan and `WL12` reads them against a floor in blocks. |
...
| `BodyEdges` | The **edge-taxonomy reader**: classifies any rectangle set's negative spaces and boundary edges from geometry alone — the derive-side, shape-relative generalization of the emit-time `ShapeVacancy` publication (works on emissions, terminal-free compounds, future hub bodies). |
...
| `FillMenu` · `FillMenuRow` | The **width→fill production rule** (§3): the one/two/three-lane table saying which families an interface width makes legal. Rungs are counted in lanes, not cells, so the table reads the same on any grid scale. `FillProfiles` composes this for the wool box. |
...
The **plan** is the author-intent layer (§4): only what a machine cannot recover. Everything structural is
...
| `PlanRoles` | The **map-level piece roles** (the *role* taxonomy, distinct from slots): `piece` (anonymous) · `wool-room` · `spawn` · `buffer`. `piece`/`wool-room`/`spawn` make terrain; `buffer` is the one annotation. |
...
| `PlanGlobals` · `PlanMeta` | The plan's **frame**: cell size, symmetry, player cap, base surface height (globals); name/metadata (meta). It carries **no build ceiling**: that was `headroom`, a slack over `surface`, computed from the flat nominal world the relief solve abandons. The author's rule measures it off the built terrain instead, in `BuildCeiling` (B105, `G6` amendment 14). |
| `BuildCeiling` | The two numbers a finished map's vertical frame is: **20** blocks of build clearance over the highest block a player meets — the terrain, and the buildings standing on it — and **5** more to a goal marker's floor. `Floating` names what is left out. In `Domain`, applied in `WorldBuilder` after the dressing pass, over the columns provenance records as `Structure` and stepping over the courses a made thing occupies (`G6` amendment 25). |
| `Finding` · `Findings` · `Severity` | **One thing wrong with something an author wrote**, in the one shape every gate in the studio answers in (`docs/refusals.md`): a stable rule id, a sentence carrying the measured numbers, and what it is about — a document field, the ids it indicts, or neither. A refusal stops the work and a complaint rides along with it. `Findings` is what a gate answers — every gate is `Check`, and `Refuses` rather than `Count` is the question, since a gate reporting complaints answers a non-empty list for a good document. Lives in `Vocabulary`, a leaf referencing nothing — the only project a gate below `Api`, the HTTP surface and the WASM client all reach, which is what lets the record be serialized rather than mirrored. |
| `PlanValidator` · `PlanRules` | The **plan-level lint**: `Finding`s (refusals vs complaints) checkable with little or no geometry, each citing a `PL*` rule where the check is the plan's own and the checked document's own id where it is not. |
...
(§6): the **shape deriver** reads one wool box's family; the **board deriver** reads the whole board's
connectivity.
...
| `PlanNav` · `PlanPoi` | The **plan as ground someone walks**: the fanned board as a navigable cell set — `Ground` (every generating piece), `Bridge` (a build zone's cells, void crossed by building), `Lane` (a water lane, drawn and not walked before it opens), the enclosed `Holes`, and every marker as a `PlanPoi`. One tier above the piece graph and one below the built world: `ContactGraph` says whether two rectangles touch and `FannedGraph` whether one reaches another, and neither can say how far, how wide, or how many ways. `Walkable()` hands those sets to the shared `Walk` as the plan tier's `WalkGround`, and `For(team)` hands over that team's own — everything `Barred(team)` takes out, which is another team's spawn and the wool room this one defends. The twin of `Analysis.WorldWalk.For` one tier down. |
| `Walk` · `WalkGround` · `WalkCost` · `WalkPath` · `WalkAim` | The **one traversal a distance is measured with** (`PgmStudio.Geom`), eight-connected and octile — a diagonal needing ground on both cells it squeezes between, so a corner over a drop is walked round — answering `(reachable, distance, blocks, drops)` — distance in blocks, blocks in blocks *placed* (a rise of Δ costs Δ−1, void bridged costs one a cell), a drop free to 3 and counted beyond it, water doubling the distance it covers. Nothing is weighted: `WalkAim` picks whether the route minimises distance (`Travel`), placed blocks (`Reach`) or exposure to an edge inside a `Detour` allowance (`Comfort`), and outside `Comfort` the clearance breaks a tie rather than being charged. `Field` prices every cell from one origin in a single solve, `Corridor` sums two fields into the ribbon of cells a route within an allowance can reach, and `Crossings` sums two into the seats the sides meet on — one per way across, each stretch's widest cell — so a coverage read and a per-pair distance cost one traversal apiece rather than one each. `WalkGround` is the per-cell answer both fidelities fill in — `PlanNav.Walkable()` from the plan, `Analysis/WorldWalk` from a scanned or built world, narrowed per team by `WorldWalk.For` where an `enter` rule bars one. |
| `PlanRoutes` · `RouteRead` · `RouteOption` · `HoleRead` · `RouteFork` | The **journey read**: from any cell to any other, the shortest walk, the corridor, and the genuinely different ways. Options are found by casting a ray from a hole to the board's edge and re-walking with it removed — what survives is a route committed to the other side — and kept only when the **piece sequence** differs. `RouteFork` is **one decision per door** — the hole it is about, where the choice is made, where the two ways meet again, how long it stays open and what the other way costs — because a board offering two choices offers them in two places and a span from the first parting to the last merge describes neither. |
| `PieceInterfaces` | The **per-interface read** over the two derivers: every land seam with its height delta and typed wall (`Seam`), each piece side's frontline share (`Frontage`), the straits between bridged islands (`IslandGap`), and the frontline runs in blocks — what the interface lints (SP8/WL11/ST8/FR8/FR9/CT12) and `/plan/inspect`'s aggregation feed quantify over. |
...
| `PlanBoardAscii` | The **plan as a grid of characters**, off the same `PlanBoardScene` the PNG and SVG draw. A plan is a list of rectangles and most of what goes wrong with one is a *relation between two of them* — a landform wider than the band that reaches it, a wall on the only throat. A render of the built world cannot show a relation between two rectangles; a grid puts them on the same rows. Carries an `Overlay` channel for a route, a corridor or a cut, and `Compare` for laying two cell sets against each other. |
| `ShapeCatalog` · `CatalogShape` · `CatalogTier` | The **vocabulary as data the studio renders**: one entry per shape the pipeline can put in a box — every approach family and body form, emitted once mouth-up, deduplicated by cell pattern, and wrapped as a standalone `symmetry:none` plan. Each carries a `CatalogTier` saying how far it gets: `InMix` (a sampler draws it), `Reachable` (`BoxFiller` fills it and the menu lists it, but nothing asks — Z, G146), `EmitterOnly` (off the menu, or a knob `WoolBoxEmitter.Fill` does not forward — the scythe, G145's five). The reachable set is **collected by running `UnitRequests.WoolRequest`** over seeds and the grid of inputs it reads, never by restating its laws, so retuning a chance or a cap in `UnitTuning` moves the catalog with it. Backs the `/catalog` page (G144). |
...
| Type | What it means |
|---|---|
| `ComposeRequest` | The **input**: players per team (6–47, clamped to the ladder's own top band), team count (2 or 4), symmetry, seed, cell size (4 by default). Validated at construction — nothing else is given, and there is no geometry input (`model.md` §2.1). |
...
The emitter can make anything; the maps' character is **what the evaluator refuses** (§8). `score = Σ
...
| `PlanFlow` · `FlowLeg` · `Approach` | **What a board asks of each side**: per objective, one `Approach` per demand set — attack, back-run, defend, chase — each read over the ground that side walks, carrying its distance, its ways and its `RouteFork`s. Each leg also carries its **interference** — the share of the defence's own corridor that the attack's covers, the measure that says whether a second way in is worth taking — which the `route-interference` term scores. Beside them the ground no journey reaches, as a share, as named patches with coordinates, and as a count of the stretches too small to name; the share is what the `dead-share` term scores. |
...
| `Producibility` · `PlanProducibility` · `BoxProducibility` · `ProducibleAs` · `NearestMiss` | **Could the composer have produced this?** — the emit↔derive mirror as an authoring answer, and the question the evaluator does not ask (a plan scores 0 and can still be unbuildable). Answers by **search, not inverse**: enumerates the tuples the declared menus admit, calls the real emitters, compares masks. Per box it reports the derived `Identity` (a hint, never a verdict), the tuple that reproduces it (`ProducibleAs`) or the `NearestMiss` and its differing cells; `PlanProducibility` adds the **unit-level** findings (the arrangement rules — parallel fronts, the frontline face demand, seat separation). Both halves answer in `Findings`, and `IsProducible` reads `Refuses` rather than a count. Inside the read a finding is a **refusal** — it is the reason the answer is no, which is what gives the record a question to ask by name; it reaches a caller as a **complaint** via `Findings.AsComplaints` at the wire, because a box the emitters cannot reproduce is still a box an author drew. `Cites` names the rule id or the task that would unblock it. |
...
| Type | What it means |
|---|---|
| `CellRect` | The **plan rectangle** — an origin plus two *exclusive* extents, in plan cells (`X`, `Z`, `Width`, `Height`, with `MaxX`/`MaxZ` computed). Every box, piece, zone and vacancy is one. Distinct from `Rect`, now its neighbour in `Geom`, and the split is the point: `Rect` is **world blocks**, fractional, an inclusive min/max corner pair; `CellRect` is **plan cells**, integer, origin + extent. A `JsonConverter` keeps the `plan.json` wire form a four-element array, so the representation changed and the serialized form did not. (B37, B210) |
| `Rect` | The **authoring rectangle** — a footprint in world XZ blocks as an inclusive corner pair (`MinX`, `MinZ`, `MaxX`, `MaxZ`), fractional because a spawn, a protection region and a wool room all sit on the half-block lattice a marker snaps to. `Covers(x, z)` is inclusive on both edges, where `CellRect.Contains` is half-open. In `Geom` since `B210`, which is what let the seven copies of it — one per Configure step that draws a footprint, plus `MapIntent` and `ResourceRenewables` — collapse to one. (B210) |
| `BlockBox` | The **block volume** — an inclusive 3-D world AABB (`MinX`/`MinY`/`MinZ` … `MaxZ`) covering every role such a volume plays: the region an author boxed for a scan, the volume a stamper fills, the casing a detector proposes, the frame a view is drawn over. `CuboidMax` is the exclusive max a PGM `<cuboid>` wants; `CentreX`/`CentreZ` the half-step centre a marker is placed at. Moved `Domain` → `Geom` by `B210` so the WASM client can reach it, which retired `CoreAuthoring.Box` and `ViewBox`. (B33, B210) |
| `Cells` · `Stretch` | The **rectilinear cell-set primitives**: neighbours, flood fill, connected components, enclosed-void detection, reflex-corner (bend) counting, fold detection, the diagonal-pinch corner law, min run width, and the **minimum vertex cut** — the cheapest cells that, held, keep two pieces of ground apart, whose count is the funnel capacity. Its ends are sets: a cut against a single cell is capped at the four ways out of it and measures nothing. Null where the ends touch, empty where they are already apart. `Stretches` cuts a cell set into the **`Stretch`es worth naming** — every 4-connected component above a floor, measured as area and centre, largest first and then by position — beside a count of the ones that fell under it, and a stretch carries its own cells so a caller reads its tier's fact off them. Both dead-ground reads take it: `PlanFlow` at a floor in blocks, `GroundCoverage` at one in cells. |
