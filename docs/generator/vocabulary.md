...
| `MouthOrient` | The **orientation** step for the terminal-capped kinds (`model.md` §6): a shape is always emitted mouth-up, and this turns it onto whichever box edge it actually docks — Top the identity, Bottom a vertical mirror, Left/Right a quarter turn. Piece rects, the marker's offset inside its rotated room, and every published vacancy mouth follow the same transform, so per-family geometry is written once instead of four times. |
...
| `TeamUnitFiller` · `FilledUnit` | The **fill half**: fills an allocated partition **hub-first**, each neighbour consuming the offer on its own joint as its corridor width, and assembles the pieces + placements. `FilledUnit` carries the `GrownUnit` plus the frontline face offers the mid consumes (`model.md` §6). |
