...
| `BuildCeiling` | The two numbers a finished map's vertical frame is: **20** blocks of build clearance over the highest *terrain* column the world builds, and **5** more to a goal marker's floor. In `Domain`, applied in `WorldBuilder`, and measured on `SurfaceTop` before a single structure is stamped — so nothing placed on the map can raise its own ceiling. |
...
| `PlanRouteCost` · `RouteCosts` | What a step costs **beyond being a step**: the edge (a walk that only minimises length hugs every border, and a border here is void), the bridge, and the reach of a held wall over bridged ground. Widths and ranges are stated in **blocks** and divided by the plan's cell, so the same numbers read at any trace scale. `RouteCosts.Flat` charges nothing and is the walk to compare against. |
| `PieceInterfaces` | The **per-interface read** over the two derivers: every land seam with its height delta and typed wall (`Seam`), each piece side's frontline share (`Frontage`), the straits between bridged islands (`IslandGap`), and the frontline runs in blocks — what the interface lints (SP8/ST8/FR8/CT12) and `/plan/inspect`'s aggregation feed quantify over. |
