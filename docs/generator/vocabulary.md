...
| `BuildCeiling` | The two numbers a finished map's vertical frame is: **20** blocks of build clearance over the highest *terrain* column the world builds, and **5** more to a goal marker's floor. In `Domain`, applied in `SketchWorldBuilder`, and measured on `SurfaceTop` before a single structure is stamped — so nothing placed on the map can raise its own ceiling. |
...
| `PlanNav` · `PlanPoi` | The **plan as ground someone walks**: the fanned board as a navigable cell set — `Ground` (every generating piece), `Bridge` (a build zone's cells, void crossed by building), `Lane` (a water lane, drawn and not walked before it opens), the enclosed `Holes`, and every marker as a `PlanPoi`. One tier above the piece graph and one below the built world: `ContactGraph` says whether two rectangles touch and `FannedGraph` whether one reaches another, and neither can say how far, how wide, or how many ways. |
