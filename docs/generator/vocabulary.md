...
| `Walk` · `WalkGround` · `WalkCost` · `WalkPath` · `WalkAim` | The **one traversal a distance is measured with** (`PgmStudio.Geom`), eight-connected and octile — a diagonal needing ground on both cells it squeezes between, so a corner over a drop is walked round — answering `(reachable, distance, blocks, drops)` — distance in blocks, blocks in blocks *placed* (a rise of Δ costs Δ−1, void bridged costs one a cell), a drop free to 3 and counted beyond it, water doubling the distance it covers. Nothing is weighted: `WalkAim` picks whether the route minimises distance (`Travel`), placed blocks (`Reach`) or exposure to an edge inside a `Detour` allowance (`Comfort`), and outside `Comfort` the clearance breaks a tie rather than being charged. `Field` prices every cell from one origin in a single solve and `Corridor` sums two fields into the ribbon of cells a route within an allowance can reach, so a coverage read and a per-pair distance cost one traversal apiece rather than one each. `WalkGround` is the per-cell answer both fidelities fill in — `PlanNav.Walkable()` from the plan, `Analysis/WorldWalk` from a scanned or built world, narrowed per team by `WorldWalk.For` where an `enter` rule bars one. |
