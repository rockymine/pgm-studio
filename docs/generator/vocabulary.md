...
| `PieceInterfaces` | The **per-interface read** over the two derivers: every land seam with its height delta and typed wall (`Seam`), each piece side's frontline share (`Frontage`), the straits between bridged islands (`IslandGap`), and the frontline runs in blocks — what the interface lints (SP8/WL11/ST8/FR8/CT12) and `/plan/inspect`'s aggregation feed quantify over. |
