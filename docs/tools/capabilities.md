...
monuments, destroyables, cores, capture points with the score they pay into, the shops a board sells from with
the keepers that open them and the spawners that mint what they are priced in, the build region and its holes,
water lanes, and `structures` — entrance redstone, iron cubes and approach walls. It is the only layer that knows the objective.
...
and previews the defaults — `flow.md` § *What a write endpoint takes* has the rule and its exceptions.

```json POST /api/terrain/theme-preview
{"rimEdges": "drop", "buckets": {}}
``` `POST /terrain/theme-preview` and `/terrain/theme-map-preview` show a theme as it will
