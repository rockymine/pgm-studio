...
| `spawners` | per generator: an id, the block it drops into, how long between drops, how many may lie uncollected and what it drops. What mints the currency a shop is priced in, since a spawn kit and the kill reward derived from it are the only other item sources a generated board has; the regions PGM's element references are minted from the id (`docs/pgm/shops.md` §10) |
