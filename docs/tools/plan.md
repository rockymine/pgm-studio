...
| `POST /plan/columns` | `{palette, cols, min_x, min_z, max_x, max_z, warnings}` — the world the plan compiles to, as per-column runs, in the encoding `sketch.md` documents, plus every prop the dressing pass declined as a `DR-*` finding under `warnings`. It compiles and builds, so it is the heaviest read here and the only one that answers what stands above the ground. It does not gate: `/plan/compile` is where a plan is refused, and a preview of an incoherent plan is still worth looking at | 400 malformed or unbuildable |
