...
| **PT4** | *A sampled field paints a face and states no vertical period.* A field of the **plane** gives every block in a column the same answer, which is a fabric read from above and a run of vertical stripes read edge-on; a `rise` is the vertical period that samples the volume instead and gives a face its grain. Asked of the **wall**, which is a face by definition and is asked only where it paints, and of the **fill** where the theme carries neither a surface nor a rim — which is a thing that is *made of* its material (`TP22`) rather than ground with a top, so every side of it is a face. A fill under a surface is ground and is not asked: it is met from above wherever it is not cut, and every committed preset paints its body with a flat `voronoi` on purpose. One band of a **depth** stack is the one place the question does not apply, since a band a single course thick has no height to vary over — the course of turf over a clay body two presets lay exactly that way. A stack read by ring, by world height or by inclination gives each band the whole span, so each is asked as the bucket was. *Measured over the ten probe boards: `probe-plains-1` raises nothing across its eight themes, `probe-plains-2` raises one on each of its five and on 31 of its 39 shape materials — the pair authored from one brief, where one agent gave every field in a wall a rise by hand and the other shipped striped tunnel walls.* |
