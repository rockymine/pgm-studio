...
that stops and the ground beside a building does not take the building's roof for a gradient.

**The gradient is read two cells either side, because a gentle slope in a voxel world is a staircase.** Ground
falling one block every four cells is four flat treads and a one-block riser, and a window one cell wide reads
each riser on its own — 27° on the riser and nothing on the treads — so a uniform gentle grade comes out
speckled with whatever the theme paints at 27, and an isolated contour in an otherwise flat field paints a
stripe across it. The tell is in the distribution rather than in the picture: read one cell either side, a
board authored at `step: 1` puts a quarter of its ground in a spike at 20–29° with almost nothing at 30–39,
which is a board reporting its own quantum and not its shape.

The width is set from both ends and the arithmetic decides it. A **sustained** slope reads its true angle at
every window — one block per four cells is 14° at 1, 2, 3 and 4 — so widening costs a real slope nothing.
What it costs is a face, whose drop is spread over more cells: the lip of a six-block drop reads 72° at one,
56° at two and 45° at three, where it can no longer be told from a walkable ramp. The isolated one-block
contour being suppressed falls 27° → 14° → 9.5° over the same range. Two is where the contour is already
under any threshold an author would set and the face is still unmistakably a face; `TerrainProfile.SlopeWindow`
is where it is written down, and `GET /map/{slug}/incline?window=` is where another can be tried without
rebuilding a board.
...
**Where the cuts go is read off the board rather than guessed.** `GET /map/{slug}/incline` answers how much
ground stands in each ten degrees, which is the only thing that says whether a cut lands where the author
...
A disc and a cross are the two shapes worth walking this on, because an inner corner seeds ring 0 as surely as
an outer one does.
...
piece of ground both teams walk onto, and the result is a map tinted red end to end.

**`PT5` is what says so.** `TeamTerritory.Shared` reads the same decomposition for the islands more than one
team's spawns stand on, and the build asks whether the paint on such an island's cells says whose land it is
— `Materials.TintsByTeam`, which walks a material tree, since a tint sits as readily inside a pattern or a
band as at the root of a bucket. Both true and the world carries a complaint naming the island, the teams that
enter it, the colour all of it wears and how many cells that is. It is asked of the themes the painter
**resolves**, on that island's own cells, so a registry theme no shape applies never raises it, and a board
with no shared island never takes the walk. A complaint rather than a refusal, because the map builds and the
answer is the author's: state ownership per structure with a shape's own theme, split the land the tint is
meant to distinguish, or leave the tint off the terrain. `IslandTeams` is populated **once, when it's
cheapest** — the `/plan/compile` endpoint
...
**The Theme phase is where a board's field is chosen.** It sits in the board defaults beside the map default
theme, because it is the same kind of statement: what every column falls back to. It is a select over the
library's rows and nothing more — the kinds, the numbers and the palette are the library editor's, where there
is room for them.
...
| **PT2** | *A pattern states a band, a stop or a side and carries no material in it.* The three pair-carrying members — a `layered`'s `stack` band, a `voronoi`'s `bands` entry and a wall run's stripe — are each a value type, so an entry naming only its depth or its width binds with an **empty** material rather than refusing to bind at all; a bare list of materials handed to `bands` by analogy with `noise`'s `stops` produces exactly that, one empty band per entry. Nothing reads the member until the painter does, by which time the world is being built, so it is looked for where the layout is stored. The finding names the path to the member (`fill.bands[1]`). |
| **PT3** | *A sampled pattern's brush is finer than the blocks it paints.* A `cell`'s and a `voronoi`'s `cellSize`, and a `noise`, `turbulence` or `electric` field's `scale`, are the period the pattern varies over in **blocks**; below two it changes faster than the ground it is laid on can show, every block is its own feature, and the pattern reads as noise at any distance however good its palette. The floor is **2** and it is a guard against a pathological number rather than a judgement about taste — how coarse a brush should be is the author's, and only a brush finer than one block is nobody's; the shipped presets sit at four to ten and a test holds every one of them silent. Asked of every pattern in the tree, so a fine field nested in a coarse voronoi's band is named where it sits (`fill.bands[1].scale`). A `checker`'s square and a wall run's stripe are **drawn** rather than sampled — geometry an author placed at the width they meant — so neither is asked. |
| **PT4** | *A sampled field paints a face and states no vertical period.* A field of the **plane** gives every block in a column the same answer, which is a fabric read from above and a run of vertical stripes read edge-on; a `rise` is the vertical period that samples the volume instead and gives a face its grain. Asked of both tall buckets. The **wall** is a face by definition, and is asked only where it paints. The **fill** is the face every cut through a board leaves — a cliff, a tunnel, a void edge — and is the bucket a shape's own `material` reaches, so it is asked the same way; a fill flat through its whole depth stripes exactly where a player stands closest to it (author). Every band of a stack is asked whatever its thickness: a course sits against the courses around it, so a field flat through one still meets its neighbours in a straight seam, and **a rise is never nought on a face**. Only a **sampled** field is asked, which is the five kinds that carry a `rise` at all — `voronoi`, `cell`, `noise`, `turbulence`, `electric`. Everything else is **drawn**: a `checker`'s square, a `logChecker`'s, a `wallRun`'s stripe, a `wallDiagonal`'s, a `wallFrame`'s edge and a `laidLog`'s axis are geometry an author placed and each already states what it does with height, while a `solid`, a `layered` and a `teamTint` vary with something that is not position. None of them has a rise to state and none is asked for one. The committed bodies give the number its scale — two or three blocks against a `cellSize` of nine or ten, so the fabric turns over every few courses without the face reading as noise. *The pair the finding was read off separates on it: `probe-plains-1` raises nothing across its eight themes, `probe-plains-2` eight across its five and on 31 of its 39 shape materials — one agent gave every field a rise by hand, the other shipped striped tunnel walls. Three of the ten probe boards raise nothing on their themes at all.* |
| **PT5** | *A team tint paints land more than one team enters.* The tint is one colour per canonical island, which is what makes it readable — a player standing anywhere on a landmass knows whose it is — so an island two teams' spawns stand on wears whichever team the ownership resolved, everywhere, and on a board whose ground is a single landmass that is the whole map. `TeamTerritory.Shared` names those islands off the same decomposition the tint paints through and `Materials.TintsByTeam` walks a material tree for the tint itself, since one sits as readily inside a pattern or a band as at a bucket's root and a bucket its own toggle turns off states no colour. Asked of the themes the painter **resolves**, on the shared island's own cells, so a registry theme no shape applies never raises it and a board with no shared island never takes the walk; the finding names the island, the teams, the colour and how many cells wear it. A **complaint on a built world**, because the map builds and the answer is the author's: ownership per structure with a shape's own theme, a split in the land, or the tint left off the terrain. |
| **TP22b** | **A tone family is a colour, and a block is also a texture.** The palette answers one mean per block, which is the right answer for a pixel and a poor one for a choice: `stone` and `cobblestone` are four counts apart per channel and differ by a factor of **2.5** in contrast, and `stone_andesite` and its polished variant are four apart and close in contrast, separated only by a bevel. So each block also carries, **per face**, the sprite it wears and what that sprite reads as — `contrast` (the standard deviation of luma over the sprite's opaque pixels), `colours` (distinct RGB values in its 256: five is a hand-picked ramp, two hundred a generated field) and `construction` (`inlaid` · `masonry` · `tiled` · `panelled` · `bevelled` · `dithered` · `ramped` · `grained-x` · `grained-y` · `flat`, not exclusive). **Two faces because a theme paints two**: the surface and rim buckets write the top, the wall and fill the side. Sandstone, smooth sandstone and the double sandstone slab all wear `sandstone_top`, so a *surface* pattern built from two of them holds one block whatever the document says, while a *wall* pattern from the same two holds two. Measured off the 1.8 textures and stored as a table (`BlockLookData`) rather than computed: the sprites are Mojang's and are not in the repository, which is what makes the numbers a result worth committing — the same division the mean colours already sit under. A biome-tinted block is greyscale in the file and tinted at render, so `grass_top` reads here as the grey it is stored as while its swatch carries the tint; `grass_side` needs no such caveat, shipping with its green fringe drawn in. Served on `GET /terrain/blocks`, with `GET /terrain/looks` naming the construction words. |
