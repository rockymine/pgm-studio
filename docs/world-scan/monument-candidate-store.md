...
three operations that used to read a world at runtime, because `segment`/`layer.parquet` cannot drive
...
**Snapping a confirmed suggestion onto buildable ground** using `segment` is downstream of this and
independent of it.
