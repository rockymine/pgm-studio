...
## 5b. One void rule, and it is the template's

**What a board says about the void is a single apply rule scoped to `not-build-area`.** `BuildGenerator`
writes the negative of the buildable region and applies `block-place="block-place-void-filter"` with
`block-break="block-break-void-filter"` over it, which is `docs/pgm/template.xml`'s shape down to the two
filter ids. A board that declares no buildable rectangle writes no void rule at all, because the boundary
the rule is scoped to is the edge of the buildable region and there is none.

**Nothing wider may ride beside it, because PGM stops at the first apply rule that decides.** A second rule
over `everywhere` is not a second opinion but the deciding one: it denies placing over the void *inside* the
rectangles the board declared buildable, which is what a build area exists to permit, and it precedes the
spawn and wool-room protections in the rule list. `IntentGenerator` emits the build slice last for the same
reason — the broad `not-build-area` rule allows editing any solid block, so it must come after the
protections that sit outside the build area.

**The corpus enforces the void about as often by a hard region as by the void filter.** Measured over the 112
`dtcm` maps in `CommunityMaps`: 102 declare a `maxbuildheight`, 68 enforce the void somewhere (56 by the
inline `deny(void)` idiom — 31 as `block-place`, 25 as `block` — the rest through a named `not(void)` filter
referenced indirectly), 82 carry a hard `block="never"` region, and 100 of 112 carry one or the other.

**Where a map does enforce by the void, it is `block-place`, not `block`.** A player may still *break* a
block hanging over the void; only *placing* one out there is denied, which is what lets a destroyable float
over open void without its own goal becoming unbreakable. `block-break-void-filter` is the studio's reading
of the same rule: it allows over ground exactly as the place side does, and over the void allows only what
the dressing stage leaves there.
...
**A shop is stated on the intent and its keepers are derived unless it says otherwise.** `shops` carries the
menu — an id, a name, its categories, and per icon the stack, every payment it costs and the action buying it
triggers. A keeper that names no place of its own is derived: one per shop at every team's spawn, beside the
point players arrive on and facing them (`shops.md` §9). One that names a block or a region stands there,
once, which is how a shop building in the middle of a board is stated. Nothing is stamped for one, because
