...
A lane sits inside `not-build-area` and that is what keeps it closed: the void rule denies placing over a
column with nothing at `y=0`, which is every lane column until the fragment floods it, and from that tick
the column is no longer void and the rule no longer bites (`new-map-authoring.md` §5b). Nothing about
`WaterLaneGenerator` changes to keep this true — it already emits the lane as a region the build slice's
union never reaches.
