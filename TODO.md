...
**One entry.** The stack-of-layers group has drained: what five authored maps
(`pgm-studio-mapgen/reports/opus5-*`) opened is built, and every ruling the author made on a stacked board is
written down where its subject lives — `docs/tools/sketch.md` § Layers for what a layer is and what the gate
says about a stack of them, `docs/tools/plan.md` § Limits for why a plan states none, and
`docs/gameplay/approaches.md` for what a second storey is played for.
